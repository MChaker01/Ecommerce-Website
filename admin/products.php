...
if(isset($_POST['add_product'])){ 
   ...
   $id_categ = filter_var($id_categ, FILTER_SANITIZE_STRING);
   /////////
   $select_categories = $conn->prepare("SELECT * FROM `categories` WHERE id_categ = ?");
   $select_categories->execute([$id_categ]);

   $select_products = $conn->prepare("SELECT * FROM `products` WHERE name = ?");
   $select_products->execute([$name]);

   if($select_products->rowCount() > 0){
      $info_msg[] = 'product name already exist!';
   }else{

      $insert_products = $conn->prepare("INSERT INTO `products`(name, details, price, image_01, image_02, image_03, id_categ) VALUES(?,?,?,?,?,?,?)");
      $insert_products->execute([$name, $details, $price, $image_01, $image_02, $image_03, $id_categ]);
...
<body>
   ...
   <form action="" method="post" enctype="multipart/form-data">
      <div class="flex">
        ...
        <div class="inputBox">
         ...
         <div class="inputBox">
            ...
            <select name="id_categ" class="box" maxlength="500" required>
               <option value="">Select a Category</option>
               <?php
                  $select_categories = $conn->prepare("SELECT * FROM categories");
                  $select_categories->execute();
                  $categs = $select_categories->fetchAll();
                  foreach ($categs as $categ)
                  {
                  echo "<option value = '" . $categ['id_categ'] . "'> " . $categ['categ_name'] . " </option> ";
                  }
               ?>
            </select>
      ...
      <input type="submit" value="Add Product" class="insert-btn" name="add_product" style="margin: 3rem auto auto;">
   </form>
...
<section class="show-products">
   ...
   <div class="box">
      ...
      <div class="price"><span><?= $fetch_products['price']; ?></span> DH</div>
